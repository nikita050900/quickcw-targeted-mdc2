#!/bin/bash
# Run F of Table 3: G2D2, fixed fGW, detection prior
#
# SLURM template. Set the partition and any account string your site needs,
# then submit with  sbatch submit_F.sh
#SBATCH --job-name=G2D2_narrow_detect_tref_4core
#SBATCH --output=G2D2_narrow_detect_tref_4core_%j.out
...
/usr/bin/time -v python "$(dirname "$0")/run_G2D2_fixed.py" \
    --save_filename G2D2_narrow_detect_tref_4core.h5 \
