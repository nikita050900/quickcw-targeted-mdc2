#!/bin/bash
# Run B of Table 3: G2D1, fixed fGW, detection prior
#
# SLURM template. Set the partition and any account string your site needs,
# then submit with  sbatch submit_B.sh
#SBATCH --job-name=G2D1_narrow_detect_4core
#SBATCH --output=G2D1_narrow_detect_4core_%j.out
...
/usr/bin/time -v python "$(dirname "$0")/run_G2D1_fixed.py" \
    --save_filename G2D1_narrow_detect_4core.h5 \
